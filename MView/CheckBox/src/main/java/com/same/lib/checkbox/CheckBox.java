...
public class CheckBox extends View {
    ...
    public CheckBox(Context context, int resId) {
        ...
        backgroundPaint.setStrokeWidth(dp(2));

        textPaint = new TextPaint(Paint.ANTI_ALIAS_FLAG);
        textPaint.setTextSize(dp(18));
        textPaint.setTypeface(CheckboxFont.getMediumTypeface(context));
    ...
    public void setVisibility(int visibility) {
        ...
        if (visibility == VISIBLE && drawBitmap == null) {
            try {
                ...
                checkBitmap = Bitmap.createBitmap(dp(size), dp(size), Bitmap.Config.ARGB_4444);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (drawBackground || progress != 0) {
            eraser2.setStrokeWidth(dp(size + 6));
            ...
            if (roundProgressCheckState < progressBounceDiff) {
                rad -= dp(2) * roundProgressCheckState / progressBounceDiff;
            } else if (roundProgressCheckState < progressBounceDiff * 2) {
                rad -= dp(2) - dp(2) * (roundProgressCheckState - progressBounceDiff) / progressBounceDiff;
            }
            if (drawBackground) {
                paint.setColor(0x44000000);
                canvas.drawCircle(getMeasuredWidth() / 2, getMeasuredHeight() / 2, rad - dp(1), paint);
                canvas.drawCircle(getMeasuredWidth() / 2, getMeasuredHeight() / 2, rad - dp(1), backgroundPaint);
            }
            ...
            if (checkedText != null) {
                ...
                checkCanvas.drawText(checkedText, (getMeasuredWidth() - w) / 2, dp(size == 40 ? 28 : 21), textPaint);
            } else {
            ...
            checkCanvas.drawCircle(getMeasuredWidth() / 2 - dp(2.5f), getMeasuredHeight() / 2 + dp(4), ((getMeasuredWidth() + dp(6)) / 2) * (1 - checkProgress), eraser2);
