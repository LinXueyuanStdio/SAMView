...
public class DefaultItemAnimator extends SimpleItemAnimator {
    private static final boolean DEBUG = SharedConfig.DEBUG_VERSION;
    ...
    public void endAnimation(RecyclerView.ViewHolder item) {
        ...
        if (mRemoveAnimations.remove(item) && SharedConfig.DEBUG_VERSION) {
            throw new IllegalStateException("after animation is cancelled, item should not be in "
                    + "mRemoveAnimations list");
        }

        //noinspection PointlessBooleanExpression,ConstantConditions
        if (mAddAnimations.remove(item) && SharedConfig.DEBUG_VERSION) {
            throw new IllegalStateException("after animation is cancelled, item should not be in "
                    + "mAddAnimations list");
        }

        //noinspection PointlessBooleanExpression,ConstantConditions
        if (mChangeAnimations.remove(item) && SharedConfig.DEBUG_VERSION) {
            throw new IllegalStateException("after animation is cancelled, item should not be in "
                    + "mChangeAnimations list");
        }

        //noinspection PointlessBooleanExpression,ConstantConditions
        if (mMoveAnimations.remove(item) && SharedConfig.DEBUG_VERSION) {
            throw new IllegalStateException("after animation is cancelled, item should not be in "
